...
# Raw-socket scanning needs CAP_NET_RAW. Capabilities live on the binary file, so
# every rebuild drops them. This target depends on `build` and re-applies the
# capability only when it is actually missing (so it's cheap to make `gui`/`scan`
# depend on it — no sudo prompt unless the binary was just rebuilt).
cap: build
	@getcap $(CLI) 2>/dev/null | grep -q cap_net_raw || sudo setcap cap_net_raw+ep $(CLI)

# Launch the GUI with PONTUS_CLI pointed at the (privileged) release CLI. Depends
# on `cap`, so a GUI-started scan always has raw-socket privilege rather than
# silently falling back to an unprivileged probe after a rebuild.
gui: cap
