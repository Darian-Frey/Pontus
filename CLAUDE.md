...
- **Next:** Phase 4 (Monitoring & plugins) — `pontus-daemon` scheduled rescans, alert rules, and the sandboxed pyo3/mlua/wasmtime plugin runtime (D-003). Smaller open items: IMP-009 (TLS 1.3-only cert capture), IMP-011 (updatable web-tech signature file).
