...
- **Next:** F-020 (the pyo3/mlua/wasmtime plugin runtime, D-003) — the stable `Finding` API with three sandboxed runners, then F-021 (first-party plugins) and F-022 (credentialed scanning). Smaller open items: IMP-009 (TLS 1.3-only cert capture); F-019 email channel.
