...
- `crates/pontus-core`, `crates/pontus-cli`, `crates/pontus-ffi`, `crates/pontus-daemon`: exist and build (Cargo workspace). `gui/`: Qt6 shell exists and builds (CMake, links `pontus-ffi`). `crates/pontus-plugins` does not exist yet.
...
- **Next:** F-019 (alert rules + delivery — builds on the daemon: evaluate a rule against the new observations after each scheduled scan, deliver once per match), then F-020 (the pyo3/mlua/wasmtime plugin runtime, D-003), F-021 (first-party plugins), F-022 (credentialed scanning). Smaller open items: IMP-009 (TLS 1.3-only cert capture).
