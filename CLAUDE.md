...
- `crates/pontus-core`, `crates/pontus-cli`, `crates/pontus-ffi`, `crates/pontus-daemon`: exist and build (Cargo workspace). `gui/`: Qt6 shell exists and builds (CMake, links `pontus-ffi`). `crates/pontus-plugins` exists with all three runners — Lua, WASM, and Python (pyo3, behind the opt-in `python` feature). F-020's runners are complete; scan-pipeline wiring is the remaining slice.
