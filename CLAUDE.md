...
- `crates/pontus-core`, `crates/pontus-cli`, `crates/pontus-ffi`, `crates/pontus-daemon`: exist and build (Cargo workspace). `gui/`: Qt6 shell exists and builds (CMake, links `pontus-ffi`). `crates/pontus-plugins` exists with the Lua and WASM runners (F-020 in progress; Python runner to come).
...
- **Next (F-020 cont.):** the **Python runner** (pyo3, trusted — links a Python interpreter, a dependency decision to flag), then wire findings into the scan pipeline/store. After F-020: F-021 (first-party plugins, incl. a guest SDK so a real cross-compiled WASM plugin demonstrates full target-aware parity), F-022 (credentialed scanning). Smaller open items: IMP-009 (TLS 1.3-only cert capture); F-019 email channel.
