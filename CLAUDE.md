...
- `crates/pontus-core`, `crates/pontus-cli`, `crates/pontus-ffi`, `crates/pontus-daemon`: exist and build (Cargo workspace). `gui/`: Qt6 shell exists and builds (CMake, links `pontus-ffi`). `crates/pontus-plugins` exists with the Lua runner (F-020 in progress; WASM/Python runners to come).
...
- **Next (F-020 cont.):** the **WASM runner** (wasmtime — the sandbox-critical untrusted path, must prove no-FS for the F-020 acceptance), then the **Python runner** (pyo3, trusted — links a Python interpreter, a dependency decision to flag), then wire findings into the scan pipeline/store. After F-020: F-021 (first-party plugins), F-022 (credentialed scanning). Smaller open items: IMP-009 (TLS 1.3-only cert capture); F-019 email channel.
