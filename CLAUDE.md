...
- **GUI consolidation (in progress):** the OS guess is now surfaced in the GUI (IMP-004, done) — an "OS" column in the inventory (latest `os_guess` added to `AssetSummary` via a `json_extract` subquery, flowing through `assets_json`) and in the per-asset observation history. Still to fold in: TLS (IMP-008) and web-tech (IMP-010), which first need the observation model extended to hold their findings.
- **Next:** TLS/web-tech into the scan + observation model + GUI (IMP-008/IMP-010), then Phase 4 (monitoring daemon + sandboxed plugins).
