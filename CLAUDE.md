...
- **Next:** Phase 1 deliverables all addressed (41-test suite). rDNS fills the hostname tier (`rdns::reverse_lookup`, `--no-rdns`); UDP scanning via connected sockets (`scan::udp`, open/closed/open|filtered, `--udp-ports`); diff is proto-aware (`PortRef`, so `tcp/53` ≠ `udp/53`). Remaining refinement: large-range send-batching for the stateless sweep (the "/16 in seconds" claim). Then Phase 2 (GUI + `pontus-ffi`) — a toolchain jump (Qt/C++ alongside Rust).
