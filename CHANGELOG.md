...
- Plugin registry with signature verification (F-026): `pontus-cli registry list|install|keygen|sign`. A registry is an `index.json` + plugin files (served over HTTP(S) raw-git URLs or a local directory); each plugin carries an ed25519 signature over its bytes (ed25519-dalek — pure Rust, no OpenSSL). `install` verifies the signature against a trusted registry public key and refuses — writing nothing — if it doesn't verify. The **index itself is bound by a detached signature** (`index.json.sig`), verified before the listing is trusted, so tampering with names/file-mappings/descriptions is caught too. `keygen`/`sign` are the maintainer tools (keys/signatures hex-encoded); `list`/`install` take `--pubkey`. In-app (GUI) browse/install is a follow-up.
...
- Credentialed scanning over SSH (F-022). `pontus-cli ssh-inventory <host> --user <u> [--key … | --password]` logs in with **user-supplied** credentials (never guessed) and gathers the host's OS and installed-package list. SSH is a shell-out to the user's own `ssh`/`sshpass` (D-006 posture — no SSH/crypto dependency added); a single read-only remote command covers dpkg/rpm/pacman/apk. Scope-enforced (F-007); records an observation (SSH up + OS) and the packages against the resolved asset (new `packages` store table). `pontus-cli packages` lists them. Passwords are read from `PONTUS_SSH_PASSWORD`, never the command line; host-key policy defaults to trust-on-first-use (`--strict-host-keys` to require a known host).

- `pontus-plugins` crate — the plugin host (F-020, D-003). A stable, serde-driven data contract (`Finding`/`Severity`/`Target`/`TargetPort`), a `PluginRunner` trait and a `PluginHost` that routes a plugin to the runner for its language and stamps the producing plugin's name, plus two runners:
  - **Lua** (mlua): a plugin defines `check(target)` and returns finding tables, decoded back via serde. Sandboxed to a curated, filesystem-free standard library (base + table/string/math/coroutine — no io/os/package/debug) with a memory limit. Example `plugins/telnet.lua`.
  - **WASM** (wasmtime): the untrusted tier, run with **no host imports at all** — a module structurally cannot touch the filesystem/network, and one that even imports a WASI call fails to instantiate. Fuel metering traps runaway loops and a memory cap bounds growth. ABI: the guest exports `memory` + `run(target_ptr, target_len) -> i64` (packed result ptr/len) and optionally `alloc`; `.wasm` and `.wat` both load.
  - **Python** (pyo3): the trusted, full-power tier (deliberately not sandboxed) — a `check(target)` taking a dict and returning finding dicts, with JSON as the interchange. **Opt-in behind the `python` Cargo feature** (links libpython), so default builds stay Python-free. Example `plugins/telnet.py`.

  All three runners are in and wired into scanning: `pontus-cli scan --plugins <dir>` loads plugins by extension (`.lua`/`.wasm`/`.wat`/`.py`), runs each against a `Target` built from every up host's observed ports/services, and persists results to a new `findings` store table (`record_finding`/`findings_for_scan`); `pontus-cli findings [--scan]` lists them. The CLI's `python` feature forwards to the Python runner. In the GUI, the New-scan dialog has a **Plugins** directory field (persisted in profiles) so GUI-launched scans run plugins, and **View ▸ Plugin findings… (Ctrl+F)** shows a scan's findings — severity-coloured and sortable, over a new `pontus_findings_json` FFI.
- Plugin host-capability model (F-021): plugins can now **actively probe** a service through host-mediated, **scope-enforced** capabilities rather than ambient network access (preserving the D-003 sandbox). The host grants a `HostCapabilities` object; the Lua runner exposes it as `pontus.http_get(url)`, and the host refuses any URL resolving outside the scan's scope before connecting (F-007). The CLI scan wires this from the scan's scope. Probing plugins: `http-header-audit.lua` (flags missing HSTS/CSP/X-Content-Type-Options/clickjacking defences and Server/X-Powered-By disclosure) and `snmp-info.lua` (flags SNMP readable with a default community and the system info it discloses). The latter adds a clean-room SNMP v2c GET capability — a dependency-free BER codec backing `pontus.snmp_get(host, community, oid)` over UDP 161, scope-enforced. Scan with `--udp-ports 161` to feed it. Also `ssh-hostkey.lua` — records each SSH host key (algo/bits/SHA-256 fingerprint) and flags weak/deprecated keys (DSA, RSA <2048), via a `pontus.ssh_hostkey(host, port)` capability that shells out to the user's `ssh-keyscan`/`ssh-keygen` (D-006; no SSH/crypto dependency added). And `smb-enum.lua` — flags SMB shares listable over a null/anonymous session, via a `pontus.smb_shares(host)` capability that shells out to the user's `smbclient` (D-006). This completes F-021's four deeper plugins (HTTP-header-audit, SNMP, SSH-host-key, SMB).
- First-party starter plugins (F-021), bundled in `crates/pontus-plugins/plugins/` (with a README): `cleartext-services.lua` flags services that carry data/credentials in the clear (HTTP, FTP, Telnet, POP3/IMAP, SNMP, LDAP, VNC, r-services), and `exposed-discovery.lua` notes reachable discovery/IoT services (UPnP/SSDP, mDNS, NetBIOS, WS-Discovery, IPP — mostly UDP). Clean-room well-known-port knowledge (C-001), unit-tested. Point a scan's `--plugins` (or the GUI Plugins field) at that directory to get findings on a typical network.
- Alert rules and delivery (F-019). After each scheduled scan the daemon diffs it against the previous scan of the same target range and evaluates rules (conditions `port_opened`/`port_closed`/`host_new`/`host_vanished`/`host_changed`/`address_moved`, with optional port/proto filters). Matching is headless in `pontus_core::alert` (`evaluate` over the diff); because a diff reports a change once, an alert fires exactly once per change, and the first scan of a range never alerts (no baseline). Delivery (in the daemon): `log`, `desktop` (notify-send), generic `webhook`, `slack`/`discord` webhook shapes, and `email` (SMTP via lettre with rustls — `[channels.email]` with starttls/implicit-tls/plaintext modes and optional auth) — failures are logged, never fatal. Config via `[[alert]]` rules + `[channels.*]` (validated at startup).
...
- GUI export save dialog: switching the format filter now updates the filename's extension to match (e.g. selecting CSV renames `pontus-report.html` → `.csv`), and the export format follows the file's actual extension rather than the filter alone — so the saved file's content and extension always agree.
