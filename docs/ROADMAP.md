...
- [x] Native OS fingerprinting with an updatable corpus (`os` module: passive p0f-style family-level guess from the SYN-ACK's TCP-option layout + TTL + window + DF bit + volunteered banner tokens, scored against a clean-room corpus that layers a user JSON file over built-in defaults; distinguishes Linux/Windows/macOS by stack signature; `pontus-cli scan` records it, `--os-corpus` updates signatures without a rebuild; D-011, C-001)
