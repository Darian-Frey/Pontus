...
**Priority:** Should · **Status:** In progress (JSON/SARIF/CSV done; HTML/PDF next)
HTML/PDF reports, SARIF 2.1, and JSON-native output. **Acceptance:** a scan exports to all three; SARIF validates against the 2.1 schema. The `export` core module builds a `ScanReport` (a per-host projection of observations + risk-ranked vulns + plugin findings + packages, joined by `asset_id`) and serialises it three ways: **JSON-native** (lossless — the differentiator Nmap lacks), **SARIF 2.1** (each vuln/finding a result with the host as location, severity→level, deduped rules, per-spec shape), and **CSV** (one row per host). `pontus-cli export --format json|sarif|csv [--scan N] [-o file]`. Live-verified end-to-end (a scan with findings exported to all three). **Remaining:** HTML/PDF report builder.
