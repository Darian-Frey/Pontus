...
**Priority:** Should · **Status:** In progress (Lua runner done; WASM + Python to come)
Stable `Finding` API; runners for Python (pyo3, trusted), Lua (mlua, built-ins), WASM (wasmtime, untrusted) (D-003). The `pontus-plugins` crate now holds the serde-driven contract (`Finding`/`Severity`/`Target`/`TargetPort`), the `PluginRunner` trait + `PluginHost` (routes a plugin to its language's runner, stamps the producing plugin's name), and the **Lua runner** (mlua) — plugins define `check(target)` and return finding tables, decoded straight back via serde; the state has a curated, filesystem-free standard library (base + table/string/math/coroutine, no io/os/package/debug) and a memory limit (D-003: Lua = lightweight built-ins). Example `plugins/telnet.lua`. **Remaining:** WASM (wasmtime, the sandbox-critical untrusted path) and Python (pyo3) runners, then scan-pipeline wiring. **Acceptance:** the same trivial plugin runs under each runner and returns structured findings; the WASM plugin cannot touch the filesystem. *(Lua slice: ✅ — runs the example plugin and is proven to have no io/os surface.)*
